...
terminal CANCELLED SSE event; the extra ``update_progress`` call here only
updates the in-memory ``Job.message`` for HTTP pollers — it does not
    ...
    for page_num, (ref, image_path, frozen_page) in enumerate(pages_to_export):
        ...
        if runner.is_cancelled(job.job_id):
            ...
            await runner.update_progress(
                ...
                total=total_pages,
                message=(
                    f"Cancelled after exporting {exported_count} of {total_pages} page(s); "
