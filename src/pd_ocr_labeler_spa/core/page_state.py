...
- ``PageLoader`` — the dispatch protocol:
  ...
  pulls in pd_book_tools. Right now the contract is loader-agnostic
  so test suites stub it without importing pd_book_tools.
...
- **The actual DocTR run.** The real ``LocalDoctrPageLoader.run_ocr``
  pulls in pd_book_tools.Page, the predictor cache from
...
def persist_page_to_file(
    *,
    ...
    page
        ...
        (a ``pd_book_tools.ocr.page.Page`` in production; a stub in
        tests).
    project
